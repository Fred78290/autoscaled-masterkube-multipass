...
kubectl create secret generic kubernetes-dashboard-certs -n ${NAMESPACE} --dry-run=client -o yaml \
...
kubectl create clusterrolebinding my-dashboard-sa --clusterrole=cluster-admin --serviceaccount=${NAMESPACE}:my-dashboard-sa \
...
if [ ${MAJOR} -gt 23 ]; then
	...
	DASHBOARD_TOKEN=$(kubectl --kubeconfig=${TARGET_CLUSTER_LOCATION}/config -n ${NAMESPACE} describe secret $(kubectl get secret -n ${NAMESPACE} --kubeconfig=${TARGET_CLUSTER_LOCATION}/config | awk '/^my-dashboard-sa-token-/{print $1}') | awk '$1=="token:"{print $2}')
fi
