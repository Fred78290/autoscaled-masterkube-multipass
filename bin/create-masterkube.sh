...
if [ "${KUBERNETES_DISTRO}" == "rke2" ]; then
    TARGET_IMAGE="${PWD}/images/${ROOT_IMG_NAME}-rke2-${KUBERNETES_VERSION}-${SEED_ARCH}"
elif [ "${KUBERNETES_DISTRO}" == "k3s" ]; then
    TARGET_IMAGE="${PWD}/images/${ROOT_IMG_NAME}-k3s-${KUBERNETES_VERSION}-${SEED_ARCH}"
else
    TARGET_IMAGE="${PWD}/images/${ROOT_IMG_NAME}-k8s-${CNI_PLUGIN}-${KUBERNETES_VERSION}-${CONTAINER_ENGINE}-${SEED_ARCH}"
fi
